 ...
 * 
 * @modified Brandon Abbott -- 
 * Methods add() and contains() now reference Encryption class for hash computation.
 * Modifications were made so that an HMAC_SHA1 is used for hashing instead.
 *
 */
public class BloomFilter<E> implements Serializable {
     ...
     * 
     * Modified function to use HMAC hashing scheme instead
     * @modified Brandon Abbott, Vijay Thurimella
     */
    public void add(E element) {
       ...
       for (int x = 0; x < k; x++) 
       {
    	   ...
    	   hash = MyUtilities.byteArrayToLong(MyUtilities.HMAC_SHA1(valString, x));
     ...
     * Instead of adding an element directly to the BloomFilter,
     * convert the element to BiGrams and then add to the Filter.
     * @param element - The element to be converted into biGrams, 
     * and then added to the filter.
     */
    public void addAsBigrams(E element)
    {
    	ArrayList<char[]> charList = 
    			MyUtilities.convertToBigrams(element.toString());
    	String s = "";
    	for (char[] a: charList)
    	{
    		s = "" + a[0] + a[1];
    ...
    /**
     * Returns the BitSet as a string of 1's and 0's
