...
public class S_WeightedJoinCondition extends AbstractJoinCondition {
	...
	public static class WeightedVisibleComponent extends GUIVisibleComponent {
		...
		private AbstractJoinCondition getJoinCondition() {
			...
			S_WeightedJoinCondition cond = new S_WeightedJoinCondition(colsLeft, colsRight, colsLeft, colsRight, distances, distances, weights, weights, emptyValues,null, props);
			cond.creator = this;
			return cond;
		}
	...
	public S_WeightedJoinCondition(DataColumnDefinition[] leftJoinColumns, DataColumnDefinition[] rightJoinColumns, DataColumnDefinition[] leftJoinColumns_Backup, DataColumnDefinition[] rightJoinColumns_Backup,
			AbstractDistance[] distances, AbstractDistance[] distances_backup, double[] weights,double[] weights_backup, double[] emptyValues, ArrayList<ImputationRule> imputationRuleSet, Map properties) {
		super(properties);
	...
	public Object clone() {
		return new S_WeightedJoinCondition(getLeftJoinColumns(), getRightJoinColumns(), getLeftJoinColumnsBackup(), getRightJoinColumnsBackup(), getDistanceFunctions(), getDistanceFunctionsBackup(), getWeights(), getWeightsBackup(), getEmptyMatchScore(), getImputationRuleSet(), getProperties());
	}
